...
export default {
  ...
  methods: {
    ...
    getTrackingList(value) {
      this.$get("/tracking/getTracking/subpackage?trackingSn=" + value).then(
        (data) => {
          this.TrackInfo = data;
    ...
    handlehandleCopy(value) {
      let { trackingId } = this.trackingSubpackageList.find(
        (v) => v.trackingNumber == value
      );
      this.$Modal.confirm({
        title: "编辑快递单号",
        onOk: () => {
    ...
    // 订单详情
    getorderInfo() {
      this.$get("/shopMaster/info?orderNo=" + this.orderNo).then((data) => {
