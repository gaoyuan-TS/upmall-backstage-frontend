<template>
    <div class="index-vue">
        ...
        <aside :class="asideClassName">
            <!-- logo -->
            <div class="logo-c">
                <img src="~assets/imgs/common/logo.png" class="logo">
                <span v-show="isShowAsideTitle">UP商城总台</span>
            </div>
            <!-- 菜单栏 -->
            <Menu class="menu" ref="asideMenu" width="100%" @on-select="gotoPage" 
            :open-names="openMenus" :active-name="currentPage" @on-open-change="menuChange">
                <!-- 动态菜单 -->
                <div v-for="(item, index) in menuItems" :key="index">
                    <Submenu v-if="item.children" :name="index">
                        <template slot="title">
                            <img class="menu-icon" :src="item.type">
                            <span v-show="isShowAsideTitle" style="color:#262626">{{item.text}}</span>
                            <div class="line"></div>
                        </template>
...
export default {
    ...
    mounted() {
        ...
        // 设置用户信息
        this.userName = this.$myStorage.get('userName')
        this.userImg = this.$myStorage.get('userImg')
    ...
    watch: {
        $route(to) {
            const name = to.name
            this.currentPage = name
            if (name == 'error' || name === 'create-good') {
                return
            }
    ...
    computed: {
        ...
        if(userInfo.role.roleName==='ROLE_MERCHANT'){
             return this.$store.state.menuItemsUser
        }else{
...
.active {
    background: @btn-bgColor-blue;
    border: 1px solid @btn-bgColor-blue;
}
.active a {
    color: #fff;
}
